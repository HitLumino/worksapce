        ...
        for(int i=0;i<n;i++)
        {
            if(vbVO[i] || vbMap[i])
            {
                ...
                if(vbMap[i])
                {
                    ...
                    cv::rectangle(im,pt1,pt2,cv::Scalar(255,49,11));
                    cv::circle(im,vCurrentKeys[i].pt,3,cv::Scalar(255,49,11),-1);
                    mnTracked++;
                }
                else // This is match to a "visual odometry" MapPoint created in the last frame
                {
                    // 通道顺序为bgr，仅当前帧能观测到的MapPoints用蓝色圆点表示，并用蓝色小方框圈住
                    cv::rectangle(im,pt1,pt2,cv::Scalar(255,240,18));
                    cv::circle(im,vCurrentKeys[i].pt,3,cv::Scalar(255,240,18),-1);
                    mnTrackedVO++;
                }
            }
        }
...
int line_extract(cv::Mat &im)
{
    cv::Mat imageMat =im.clone();
      ...
      /* create a random binary mask */
        ...
        for ( size_t i = 0; i < lines.size(); i++ )
        {
            ...
            if( kl.octave == 0)
            {
              ...
              /* draw line */
              line( output, pt1, pt2, Scalar( B, G, R ), 1.5 );
            }
        }
        output.copyTo(im);
        return 0;
